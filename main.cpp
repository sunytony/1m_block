...
int check_pkt(uint8_t* pkt, int size){
	...
	host_url = (char*)(http_pkt + indexOfHost + 8);
	int index = 0;
	for(;host_url[index] != 0x0d; ++index);
	host_url[index] = 0xa;
	host_url[index + 1] = 0;

	printf("host : %s\n",host_url);

	if(st.find(hash_func(host_url)) != st.end()){
...
int main(int argc, char **argv)
{
	...
	while(1){
		...
		for(;s[i]!=',';++i);
		st.insert(hash_func(s + i + 1));
	}
	end = clock();

	printf("%f clock", (float)(end - start)/CLOCKS_PER_SEC);
